...
public class SpringDataJDBCDemo implements CommandLineRunner {
    ...
    public void run(String... strings) {
        ...
        try {
            ...
            jdbcTemplate.execute("CREATE TABLE practice (id BIGINT AUTO_INCREMENT PRIMARY KEY," +
        ...
        for (Object[] name : splitUpNames) {
            jdbcTemplate.execute(String.format("INSERT INTO practice(first_name, last_name) VALUES ('%s','%s')", name[0], name[1]));
        }

        //query the database for Employees with first name Java
        jdbcTemplate.query(
                        "SELECT id, first_name, last_name FROM practice WHERE first_name = 'Java'",
                        (rs, rowNum) -> new practice(rs.getLong("id"), rs.getString("first_name"), rs.getString("last_name"))
                )
                //print each found employee to the console
                .forEach(practice -> System.out.println(practice.toString()));
